...
namespace YimMenu
{
	...
	void Vector3CommandItem::Draw()
	{
		...
		{
			ImGui::SameLine();
			if (ImGui::Button("Mevcut"))
				m_Command->SetState(Self::GetPed().GetPosition());
		}
		ImGui::SameLine();
		if (ImGui::Button("Kayıtlı..."))
			ImGui::OpenPopup("##saved");

		if (ImGui::BeginPopup("##saved", ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize))
		{
			ImGui::Text("Bir konumu seçmek için tıkla. Işınlanma > Kayıtlı bölümünden daha fazla ekle");
			InputTextWithHint("##filter", "Ara", &m_CurrentFilter).Draw();
