...
namespace YimMenu::Submenus
{
	std::shared_ptr<Category> BuildMiscMenu()
	{
		...
		misc->AddItem(std::make_unique<ImGuiItem>([] {
			...
			ImGui::InputInt("İç Mekan Dizini", &interiorIndex);

			static bool enterOwnerInterior = false;
			ImGui::Checkbox("Sahibin İç Mekanına Gir", &enterOwnerInterior);

			if (ImGui::Button("Işınla"))
			{
				FiberPool::Push([] {
			...
			static int team;
			ImGui::InputInt("Takım", &team);
			if (ImGui::Button("fm_mission_controller Takım Değiştir"))
			{
				FiberPool::Push([] {
