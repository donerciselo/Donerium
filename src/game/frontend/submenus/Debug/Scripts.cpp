				...
				for (int i = 0; i < bytesPerRow; ++i)
				{
					...
					if (auto byte = program->GetCodeAddress(index))
					{
						...
						if (ImGui::IsItemActive() && ImGui::IsItemHovered())
							ImGui::SetTooltip("Yazmak için ENTER'a bas.");
	...
	std::shared_ptr<Category> BuildScriptsMenu()
	{
		...
		threads->AddItem(std::make_unique<ImGuiItem>([] {
			...
			if (!Pointers.ScriptThreads || Pointers.ScriptThreads->size() == 0)
			{
				curThread = nullptr;
				curProgram = nullptr;
				return ImGui::TextDisabled("Yok");
			}

			if (ImGui::BeginCombo("İş Parçacığı", curThread ? curThread->m_ScriptName : "(Seç)"))
			{
			...
			ImGui::Combo("Durum", (int*)&curThread->m_Context.m_State, "Boşta\0Çalışıyor\0Öldürüldü\0Duraklatıldı\0Bilinmeyen4\0");

			if (curThread->m_Context.m_State == rage::scrThread::State::KILLED)
			{
				ImGui::Text("Çıkış Nedeni: %s", curThread->m_ErrorMessage);
			}
			else
			{
				if (ImGui::Button("Öldür"))
				{
					curThread->Kill();
					curThread->m_Context.m_State = rage::scrThread::State::KILLED;
				}
				ImGui::SameLine();
				if (ImGui::Button("Etiketleri Günlükle"))
				{
					FiberPool::Push([] {
				...
				if (ImGui::TreeNode("Bilgi"))
				{
					...
					ImGui::BeginGroup();
					ImGui::Text("İş Parçacığı ID: %d", curThread->m_Context.m_ThreadId);
					ImGui::Text("Yığın Boyutu: %d", curThread->m_Context.m_StackSize);
					ImGui::Text("Yığın İşaretçisi: 0x%X", curThread->m_Context.m_StackPointer);
					ImGui::Text("Program Sayacı: 0x%X", curThread->m_Context.m_ProgramCounter); // This is not really accurate (always points to the WAIT)
					ImGui::Text("Kod Boyutu: %d", curProgram->m_CodeSize);
					ImGui::EndGroup();
					ImGui::SameLine();
					ImGui::BeginGroup();
					ImGui::Text("Argüman Sayısı: %d", curProgram->m_ArgCount);
					ImGui::Text("Yerel Sayı: %d", curProgram->m_LocalCount);
					ImGui::Text("Genel Sayı: %d", curProgram->m_GlobalCount);
					ImGui::Text("Native Sayısı: %d", curProgram->m_NativeCount);
					ImGui::Text("Dize Sayısı: %d", curProgram->m_StringsCount);
					ImGui::EndGroup();
					ImGui::TreePop();
				}
				if (ImGui::TreeNode("Bayt Kodu"))
				{
		...
		script->AddItem(std::make_unique<ImGuiItem>([] {
			...
			bool modified = ImGui::InputTextWithHint("Betik Adı", "Ara", &scriptSearch);

			if (ImGui::BeginCombo("Yığın Boyutu", stackSizeName.c_str()))
			{
			...
			if (ImGui::InputInt("Argüman Sayısı", &argCount))
			{
			...
			ImGui::Checkbox("Başlattıktan Sonra Duraklat", &pauseAfterStarting);

			if (ImGui::Button("Betiği Başlat"))
			{
				FiberPool::Push([] {
					...
					{
						Notifications::Show("Betiği Başlat", "Betik mevcut değil.", NotificationType::Error);
						return;
					}

					if (SCRIPT::GET_NUMBER_OF_THREADS_RUNNING_THE_SCRIPT_WITH_THIS_HASH(hash) > 0)
					{
						Notifications::Show("Betiği Başlat", "Betik zaten çalışıyor.", NotificationType::Error);
						return;
					}

					if (MISC::GET_NUMBER_OF_FREE_STACKS_OF_THIS_SIZE(stackSize) == 0)
					{
						Notifications::Show("Betiği Başlat", "Bu boyutta boş yığın yok.", NotificationType::Error);
						return;
					}
					...
					Notifications::Show("Betiği Başlat", std::format("{} ID'li betik başlatıldı.", id), NotificationType::Success);
				});
			}
			...
			{
				ImGui::SameLine();
				if (ImGui::Button("Oturum Betiğini Başlat"))
				{
					FiberPool::Push([] {
				...
				ImGui::SameLine();
				if (ImGui::Button("Olaylı Betik Başlat"))
				{
					FiberPool::Push([] {
