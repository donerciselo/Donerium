...
namespace YimMenu::Submenus
{
	    ...
	    Submenu::Submenu("Network", ICON_FA_ROUTE)
	{
		...
		joinSession->AddItem(std::make_shared<CommandItem>("joinsession"_J, "Katıl##session"));
		...
		joinGroup->AddItem(std::make_shared<ImGuiItem>([] {
			...
			ImGui::SameLine();
			if (ImGui::Button("Katıl##username"))
				FiberPool::Push([] {
					...
					{
						Notifications::Show("Katılımcı", "Kullanıcı adından RID alınamadı", NotificationType::Error);
					}
				});
