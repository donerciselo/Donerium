...
namespace YimMenu::Submenus
{
	...
	static std::string GetEventStateString()
	{
		...
		case eRandomEventState::INACTIVE:
			return "Pasif - " + GSBDRandomEvents->EventData[selectedEvent].TimerState.GetRemainingTimeStr(FMRandomEvents->EventData[selectedEvent].InactiveTime) + " içinde başlatılacak";
		case eRandomEventState::AVAILABLE:
			return "Mevcut - " + GSBDRandomEvents->EventData[selectedEvent].TimerState.GetRemainingTimeStr(FMRandomEvents->EventData[selectedEvent].AvailableTime) + " içinde kapatılacak";
	...
	static void KillActiveEvent()
	{
		...
		{
			Notifications::Show("Rastgele Olaylar", "Olay betiği aktif değil. Katılımcı mısın?", NotificationType::Error);
		}
	}
	...
	std::shared_ptr<Category> BuildRandomEventsMenu()
	{
		...
		menu->AddItem(std::make_unique<ImGuiItem>([] {
			...
			if (!GPBDFM2 || !GSBDRandomEvents)
				return ImGui::Text("Freemode global bloğu yüklenmedi.");

			if (GPBDFM2->Entries[Self::GetPlayer().GetId()].RandomEventsClientData.InitState != eRandomEventClientInitState::INITIALIZED)
				return ImGui::Text("Rastgele Olaylar başlatılmadı.");

			if (auto freemode = Scripts::FindScriptThread("freemode"_J))
			{
				FMRandomEvents = RANDOM_EVENTS_FREEMODE_DATA::Get(freemode);
				if (!FMRandomEvents)
					return ImGui::Text("Freemode yığını geçerli değil.");
			}
			else
			{
				return ImGui::Text("Freemode çalışmıyor.");
			}

			if (ImGui::BeginCombo("Olay Seç", randomEventNames[selectedEvent]))
			{
				for (int event = DRUG_VEHICLE; event < MAX_EVENTS; event++)
				{
			...
			if (ImGui::InputInt(std::format("Konum Seç (0-{})", numSubvariations).c_str(), &selectedSubvariation))
			{
			...
			if (numActiveEvents >= maxActiveEvents)
				ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "Aktif Olaylar: %d/%d", numActiveEvents, maxActiveEvents);
			else
				ImGui::Text("Aktif Olaylar: %d/%d", numActiveEvents, maxActiveEvents);

			if (ImGui::Button("Olayı Başlat"))
			{
				FiberPool::Push([] {
					if (GSBDRandomEvents->EventData[selectedEvent].State != eRandomEventState::ACTIVE)
					{
						...
						{
							Notifications::Show("Rastgele Olaylar", "Olay başlatılamadı. Freemode hostu musun?", NotificationType::Error);
						}
					}
					else
					{
						Notifications::Show("Rastgele Olaylar", "Olay zaten aktif.", NotificationType::Error);
					}
				});
			}
			if (ImGui::IsItemHovered())
				ImGui::SetTooltip("Freemode betik hostu gerektirir.");

			ImGui::SameLine();

			if (ImGui::Button("Olayı Öldür"))
			{
				FiberPool::Push([] {
					...
					{
						Notifications::Show("Rastgele Olaylar", "Olay aktif değil.", NotificationType::Error);
					}
				});
			}
			...
			if (ImGui::Button("Olayın Olduğu Yere Işınla"))
			{
				FiberPool::Push([] {
					if (GSBDRandomEvents->EventData[selectedEvent].State >= eRandomEventState::AVAILABLE)
					{
						...
						{
							Notifications::Show("Rastgele Olaylar", "Olayın olduğu yere ışınlanamadı. Koordinatlar geçerli değil.", NotificationType::Error);
						}
					}
					else
					{
			...
			if (GSBDRandomEvents->EventData[selectedEvent].State == eRandomEventState::ACTIVE)
			{
				if (auto eventThread = Scripts::FindScriptThread(randomEventScripts[static_cast<int>(selectedEvent)]))
				{
					if (auto netComponent = reinterpret_cast<GtaThread*>(eventThread)->m_NetComponent)
					{
						...
						if (ImGui::SmallButton("Kontrolü Al"))
						{
			...
			ImGui::Text("Durum: %s", GetEventStateString().c_str());
			if (GSBDRandomEvents->EventData[selectedEvent].State == eRandomEventState::INACTIVE)
			{
				ImGui::Text("Konum: Yok");
				ImGui::Text("Tetikleme Menzili: Yok");
			}
			else
			{
				ImGui::Text("Konum: %d", GSBDRandomEvents->EventData[selectedEvent].Subvariation);
				ImGui::Text("Tetikleme Menzili: %.2f", GSBDRandomEvents->EventData[selectedEvent].TriggerRange); // Default value is 400, it will be updated once the event switches to the available state
			}

			// We should probably put this into a separate group, but I just don't want to do the same safety checks before rendering it
			ImGui::SeparatorText("Bekleme ve Kullanılabilirlik");

			ImGui::InputInt("##cooldown", &setCooldown);
			ImGui::SameLine();
			if (ImGui::Button("Beklemeyi Ayarla"))
			{
			...
			ImGui::SameLine();
			if (ImGui::Button("Kullanılabilirliği Ayarla"))
			{
			...
			ImGui::Checkbox("Dakika Cinsinden Uygula", &applyInMinutes);
		}));
