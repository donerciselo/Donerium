...
namespace YimMenu::Submenus
{
	...
	static void RenderPlayerList()
	{
		...
		ImGui::InputTextWithHint("Ara", "Ara", g_NameToSearch, sizeof(g_NameToSearch));
		...
		if (ImGui::BeginListBox("###player-list", {180, -100 /* static_cast<float>(*Pointers.ScreenResY - 700 - 38 * 4) */}))
		{
			...
			if (players.size() == 0)
			{
				ImGui::TextDisabled("Kayıtlı oyuncu yok");
				ImGui::EndListBox();
				return;
			}
	...
	static void RenderPlayerEditor()
	{
		...
		if (ImGui::BeginChild("##player-editor", {500, -100 /* static_cast<float>(*Pointers.ScreenResY - 688 - 38 * 4) */}, 0, ImGuiWindowFlags_NoBackground))
		{
			...
			if (ImGui::InputText("Ad", g_SelectedPlayerName, sizeof(g_SelectedPlayerName)))
			{
			...
			ImGui::Checkbox("Oyuncuyu Takip Et", &g_SelectedPlayer->m_TrackPlayer);

			if (g_SelectedPlayer->m_FetchedData)
			{
				auto& data = *g_SelectedPlayer->m_FetchedData;
				ImGui::Text("Oturum Türü: %s", FetchedPlayerData::GameStateToString(data.m_GameState).data());
				ImGui::Text("Oturum Hostu: %s", data.m_HostOfSession ? "Evet" : "Hayır");
				ImGui::Text("İzliyor: %s", data.m_Spectating ? "Evet" : "Hayır");
				ImGui::Text("Görev Lobisi: %s", data.m_InTransition ? "Evet" : "Hayır");
				ImGui::Text("Görev Lobisi Hostu: %s", data.m_HostOfTransition ? "Evet" : "Hayır");
				if (data.m_MissionType != FetchedPlayerData::MissionType::NONE)
				{
					ImGui::Text("Görev Türü: %s", FetchedPlayerData::MissionTypeToString(data.m_MissionType).data());
					if (data.m_MissionName)
						ImGui::Text("Görev Adı: %s", data.m_MissionName->data());
					else
			...
			{
				ImGui::TextDisabled("Veriler henüz getirilmedi");
			}

			if (ImGui::Button("Katıl"))
			{
				FiberPool::Push([] {
	...
	static void RenderAddNewPlayer()
	{
		...
		ImGui::SetNextItemWidth(180.0f);
		ImGui::InputText("Kullanıcı Adı", name_buf, sizeof(name_buf));
		ImGui::SameLine();
		if (ImGui::Button("Ekle"))
			FiberPool::Push([] {
				...
				{
					Notifications::Show("Kayıtlı Oyuncular", "Kullanıcı adından RID alınamadı", NotificationType::Error);
				}
			});
	}
