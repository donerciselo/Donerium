...
namespace YimMenu::Submenus
{
	...
	static bool RenderStatEditor(StatValue& value, sStatData* data)
	{
		...
		{
		case sStatData::Type::_BOOL:
			return ImGui::Checkbox("Değer", &value.m_AsBool);
		case sStatData::Type::FLOAT:
			return ImGui::InputFloat("Değer", &value.m_AsFloat);
		case sStatData::Type::INT:
			return ImGui::InputInt("Değer", &value.m_AsInt);
		case sStatData::Type::UINT32:
			return ImGui::InputScalar("Değer", ImGuiDataType_U32, &value.m_AsInt);
		case sStatData::Type::UINT16:
			return ImGui::InputScalar("Değer", ImGuiDataType_U16, &value.m_AsInt);
		case sStatData::Type::UINT8:
			return ImGui::InputScalar("Değer", ImGuiDataType_U8, &value.m_AsInt);
		case sStatData::Type::INT64:
			return ImGui::InputScalar("Değer", ImGuiDataType_S64, &value.m_AsInt);
		case sStatData::Type::UINT64:
			return ImGui::InputScalar("Değer", ImGuiDataType_U64, &value.m_AsInt);
		case sStatData::Type::STRING:
			return ImGui::InputText("Değer", value.m_AsString, sizeof(value.m_AsString));
		default:
			ImGui::BeginDisabled();
			ImGui::Text("Veri türü desteklenmiyor");
			ImGui::EndDisabled();
	...
	static bool RenderPackedStatEditor(StatValue& value, const PackedStatInfo& info)
	{
		ImGui::SetNextItemWidth(150.f);
		if (info.m_IsBoolStat)
			return ImGui::Checkbox("Değer##packed", &value.m_AsBool);
		else
			return ImGui::InputScalar("Değer##packed", ImGuiDataType_U8, &value.m_AsInt);
	}
	...
	std::shared_ptr<Category> BuildStatEditorMenu()
	{
		...
		normal->AddItem(std::make_unique<ImGuiItem>([] {
			...
			if (ImGui::InputText("Ad", stat_buf, sizeof(stat_buf)))
			{
			...
			if (!current_info.IsValid())
				return ImGui::TextDisabled("Stat bulunamadı");
			else if (current_info.m_Normalized)
			{
				ImGui::Text("Normalleştirilen ad: %s", current_info.m_Name.data());
			}
			...
			if (!can_edit && ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
				ImGui::SetTooltip("Bu stat istemci tarafından düzenlenmemelidir. Yine de yazmaya zorlamak için sağ tıkla");
			ImGui::EndDisabled();
		}));
		...
		packed->AddItem(std::make_unique<ImGuiItem>([] {
			...
			if (ImGui::InputInt("Dizin", &current_info.m_Index))
			{
			...
			if (!current_info.IsValid())
				return ImGui::TextDisabled("Dizin geçerli değil");

			RenderPackedStatEditor(value, current_info);

			if (ImGui::Button("Yenile##packed"))
				ReadPackedStat(value, current_info);
			ImGui::SameLine();
			if (ImGui::Button("Yaz##packed"))
				FiberPool::Push([] {
		...
		packed_range->AddItem(std::make_unique<ImGuiItem>([] {
			...
			ImGui::SetNextItemWidth(150.f);
			ImGui::InputInt("Başlangıç", &start);
			ImGui::SameLine();
			ImGui::SetNextItemWidth(150.f);
			ImGui::InputInt("Bitiş", &end);
			ImGui::SetNextItemWidth(150.f);
			ImGui::InputScalar("Değer##packed_range", ImGuiDataType_U8, &value);
			ImGui::SameLine();
			if (ImGui::Button("Yaz##packed_range"))
				FiberPool::Push([] {
