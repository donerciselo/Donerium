...
namespace YimMenu::Submenus
{
		...
		FiberPool::Push([&info] {
			...
			Scripts::RunAsScript(Scripts::FindScriptThread("shop_controller"_J), [&] {
				if (info.m_Type == TransactionInfo::Type::BASKET)
				{
					...
					{
						Notifications::Show("İşlemler", "Sepet oluşturulamadı", NotificationType::Error);
						txn_failed = true;
						NETSHOPPING::NET_GAMESERVER_BASKET_END();
						return;
					}
					...
					for (auto& item : info.m_Basket.m_BasketItems)
					{
						...
						{
							Notifications::Show("İşlemler",
							    std::format("{} (x{}) sepete eklenemedi", item.m_PrimaryItem.m_Name, item.m_Quantity),
				...
				else
				{
					...
					{
						Notifications::Show("İşlemler", "Hizmet oluşturulamadı", NotificationType::Error);
						txn_failed = true;
						return;
					}
				}
				...
				{
					Notifications::Show("İşlemler", "Ödeme başlatılamadı", NotificationType::Error);
					txn_failed = true;
					return;
				}
			});
			...
			if (txn && txn->m_Running)
			{
				...
				if (txn->m_Status == 3)
				{
					Notifications::Show("İşlemler", "İşlem tamamlandı", NotificationType::Success);
				}
				else
				{
					Notifications::Show("İşlemler", "İşlem başarısız", NotificationType::Error);
				}
			}
		});
	}
	...
	static bool EditTransactionItem(std::string_view label, TransactionInfo& info, TransactionItemHash& item, bool& is_valid, bool required = true, bool validate_category = true)
	{
		...
		if (
		    required ?
		        ...
		        ImGui::InputTextWithHint(label.data(), "İsteğe Bağlı", item.m_Name, sizeof(item.m_Name)))
		{
		...
		{
			SetTransactionError("Öğe bulunamadı!");
			is_valid = false;
			return false;
		}
		...
		{
			SetTransactionError("Bu öğe güvenliğin için engellendi");
			is_valid = false;
			return false;
		}
		...
		if (validate_category && !IsPriceModifier(item.m_IntendedCategory) && item.m_IntendedCategory != info.m_Category.m_Hash && !empty)
		{
			if (info.m_Type == TransactionInfo::Type::SERVICE)
			{
				SetTransactionError(std::format("Öğe kategorisi {} işlem kategorisi {} ile uyuşmuyor", CategoryNameFromHash(item.m_IntendedCategory), info.m_Category.m_Name));
				is_valid = false;
				return false;
			}
			else
			{
				SetTransactionWarning(std::format("Öğe kategorisi {} işlem kategorisi {} ile uyuşmuyor", CategoryNameFromHash(item.m_IntendedCategory), info.m_Category.m_Name));
			}
		}
	...
	static void RenderBasketEditor(TransactionInfo& info, bool& txn_valid)
	{
		...
		for (int i = 0; i < info.m_Basket.m_BasketItems.size(); i++)
		{
			...
			{
				if (EditTransactionItem("Öğe", info, item.m_SecondaryItem, txn_valid, true, false)) // TODO: validate if this corresponds to the correct non-inventory category
					item.m_Price = item.m_SecondaryItem.m_IntendedPrice;
				EditTransactionItem("Envanter Yuvası", info, item.m_PrimaryItem, txn_valid, true, true);
			}
			else
			{
				if (EditTransactionItem("Öğe", info, item.m_PrimaryItem, txn_valid, true, true))
					item.m_Price = item.m_PrimaryItem.m_IntendedPrice;
				EditTransactionItem("İkincil Öğe", info, item.m_SecondaryItem, txn_valid, false, false); // TODO: is this ever used outside of inventory stuff?
			}

			ImGui::SetNextItemWidth(180.0f);
			if (ImGui::InputScalar("Miktar", ImGuiDataType_U32, &item.m_Quantity))
			{
			...
			ImGui::SetNextItemWidth(180.0f);
			ImGui::InputInt("Fiyat", &item.m_Price);

			ImGui::SetNextItemWidth(180.0f);
			ImGui::InputInt("Stat Değeri", &item.m_StatValue); // I'm not actually sure what this does ngl

			if (info.m_Basket.m_BasketItems.size() > 1 && ImGui::Button("Sil"))
				item_to_delete = i;
			ImGui::PopID();
	...
	{
		if (EditTransactionItem("Öğe", info, info.m_Service.m_Item, txn_valid))
		...
		{
			ImGui::SetNextItemWidth(180.0f);
			ImGui::InputInt("Fiyat", &info.m_Service.m_Price);
			if (info.m_Service.m_Price > info.m_Service.m_Item.m_IntendedPrice && info.m_Action.m_Hash == "NET_SHOP_ACTION_EARN"_J)
			{
				SetTransactionError(std::format("Öğe fiyatı izin verilen maksimumu aşıyor ({})", info.m_Service.m_Item.m_IntendedPrice));
				txn_valid = false;
			}
		}
	}
	...
	{
		auto menu = std::make_shared<Category>("İşlemler");
		auto normal = std::make_shared<Group>("Tetikleyici");

		normal->AddItem(std::make_unique<ImGuiItem>([] {
			if (!NativeInvoker::AreHandlersCached())
				return ImGui::TextDisabled("Native'ler henüz önbelleğe alınmadı");

			if (AnticheatBypass::IsFSLProvidingLocalSaves())
				return ImGui::TextDisabled("FSL yerel kayıtları etkinken işlemler desteklenmez");

			if (!NETSHOPPING::NET_GAMESERVER_CATALOG_IS_VALID())
				return ImGui::TextDisabled("Katalog henüz yüklenmedi");

			ImGui::Text("Uyarı: Bu araçla yaptıklarından yalnızca sen sorumlusun. Ne yaptığını bilmiyorsan büyük ihtimalle banlanırsın");

			static TransactionInfo info{};
			bool txn_valid{true};

			ImGui::SetNextItemWidth(180.0f);
			if (ImGui::Combo("Tür", reinterpret_cast<int*>(&info.m_Type), "Sepet\0Hizmet\0"))
				OnTransactionTypeChanged(info);

			ImGui::SetNextItemWidth(250.0f);
			if (ImGui::BeginCombo("Kategori", info.m_Category.m_Name))
			{
			...
			ImGui::SetNextItemWidth(250.0f);
			if (ImGui::BeginCombo("Eylem", info.m_Action.m_Name))
			{
			...
			if (ImGui::Button("Tetikle"))
				FiberPool::Push([] {
					ProcessTransaction(info);
				});
			if (!txn_valid && ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
				ImGui::SetTooltip("İşlem geçerli değil. Tüm alanların doğru doldurulduğundan emin ol");
			ImGui::EndDisabled();
		}));
