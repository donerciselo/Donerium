...
namespace YimMenu
{
	...
	public:
		...
		void RenderOutfitList()
		{
			ImGui::BeginGroup();
			{
				...
				if (ImGui::BeginCombo("", folder.empty() ? "Kök" : folder.c_str()))
				{
				...
				if (ImGui::InputTextWithHint("###outfitname", "Ara", &search))
		...
		{
			if (ImGui::Button("Kıyafeti Kaydet"))
				FiberPool::Push([saveToNewFolder, this] {
					...
					if (!fileName.size())
					{
						Notifications::Show("Kıyafet", "Dosya adı boş!", NotificationType::Warning);
						return;
					}
		...
		void RenderOutfitListControls()
		{
			ImGui::BeginGroup();
			{
				...
				ImGui::Checkbox("Saçı Uygula", &applyHair);
				ImGui::Spacing();
				if (ImGui::Button("Seçili Kıyafeti Uygula"))
					FiberPool::Push([this] {
	...
	std::shared_ptr<Category> CreateOutfitsMenu()
	{
		...
		category->AddItem(std::make_shared<ImGuiItem>([] {
			if (ImGui::Button("Verileri Yenile"))
				FiberPool::Push([] {
					editor.RefreshStats();
				});
			ImGui::SameLine();
			if (ImGui::Button("Kıyafeti Rastgeleleştir"))
				FiberPool::Push([] {
