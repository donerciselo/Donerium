...
namespace YimMenu::Submenus
{
	...
	static void RenderAmmuNationMenu()
	{
		...
		static std::string selectedWeapon{"Seç"};
		...
		ImGui::BeginCombo("Silahlar", selectedWeapon.c_str());
		...
		{
			ImGui::Text("Şununla Öldürme: %d", kills);
			ImGui::Text("Şununla Ölümler: %d", deaths);
			ImGui::Text("K/D Oranı: %.2f", kdRatio);
			ImGui::Text("Kafa Vuruşları: %d", headshots);
			ImGui::Text("İsabet: %d%%", accuracy);
		}
	}
