...
namespace YimMenu
{
	...
	static void DrawStyleVec2(const char* label, float& x, float& y, float min, float max)
	{
		...
		bool changed = false;
		changed |= ImGui::SliderFloat(tr(PrettyPrintLabel(nameX)).c_str(), &newX, min, max, "%.1f");
		changed |= ImGui::SliderFloat(tr(PrettyPrintLabel(nameY)).c_str(), &newY, min, max, "%.1f");
	...
	static void DrawStyleFloat(const char* label, float& v, float min, float max)
	{
		...
		if (ImGui::SliderFloat(tr(PrettyPrintLabel(name)).c_str(), &newVal, min, max, "%.1f"))
		{
	...
	{
		bool changed = false;
		ImGui::Text("Köşe Eğriliği Ayarları:");
		ImGui::Separator();
		for (auto& [k, v] : g_RoundingValues)
			if (ImGui::SliderFloat(tr(PrettyPrintLabel(k)).c_str(), &v, 0.0f, 20.0f, "%.1f"))
				changed = true;
	...
	static void DrawFontTab()
	{
		...
		ImGui::Text("Yazı Tipi Yapılandırması:");
		ImGui::Separator();
		ImGui::Text("Güncel Ölçek: %.2f", io.FontGlobalScale);
		ImGui::SliderFloat("Yazı Ölçeği", &scale, 0.5f, 2.0f, "%.2f");
		if (ImGui::Button("Yazı Ölçeğini Uygula"))
	...
	std::shared_ptr<Category> DrawGUISettingsMenu()
	{
		...
		imGuiCustomStyle->AddItem(std::make_unique<ImGuiItem>([] {
			ImGui::Text("ImGui Stil Düzenleyici");
			ImGui::Separator();
			if (ImGui::BeginTabBar("StyleTabs"))
			{
				if (ImGui::BeginTabItem("Renkler"))
				{
					DrawColorsTab();
					ImGui::EndTabItem();
				}
				if (ImGui::BeginTabItem("Köşeler"))
				{
					DrawRoundingTab();
					ImGui::EndTabItem();
				}
				if (ImGui::BeginTabItem("Düzen"))
				{
					DrawLayoutTab();
					ImGui::EndTabItem();
				}
				if (ImGui::BeginTabItem("Kenarlık"))
				{
					DrawBorderTab();
					ImGui::EndTabItem();
				}
				...
				if (ImGui::BeginTabItem("Yazı Tipleri"))
				{
					DrawFontTab();
					ImGui::EndTabItem();
				}
