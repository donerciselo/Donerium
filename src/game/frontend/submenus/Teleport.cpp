...
namespace YimMenu::Submenus
{
	...
	void RenderCustomTeleport()
	{
		...
		{
			ImGui::Text("Silmek istediğine emin misin? %s", locationToDelete.name.data());
		...
		InputTextWithHint("Kategori", "Kategori", &category).Draw();

		ImGui::PushItemWidth(200);
		InputTextWithHint("Konum Adı", "Yeni konum", &newLocationName).Draw();
		ImGui::PopItemWidth();

		if (ImGui::Button("Mevcut Konumu Kaydet")) // Button widget still crashes
		{
			FiberPool::Push([=] {
				if (newLocationName.empty())
				{
					Notifications::Show("Özel Işınlanma", "Lütfen geçerli bir ad gir", NotificationType::Warning);
				}
				else if (SavedLocations::GetSavedLocationByName(newLocationName))
				{
					Notifications::Show("Özel Işınlanma", std::format("{} adında bir konum zaten mevcut", newLocationName));
				}
				else
				{
		...
		ImGui::Text("Işınlanmak için çift tıkla\nSilmek için Shift + tıkla");

		ImGui::Spacing();

		static std::string filter{};
		InputTextWithHint("##filter", "Arama", &filter).Draw();

		ImGui::BeginGroup();
		ImGui::Text("Kategoriler");
			...
			if (SavedLocations::GetAllSavedLocations().find(category) != SavedLocations::GetAllSavedLocations().end())
			{
				...
				for (const auto& l : current_list)
				{
					...
					if (ImGui::IsItemHovered())
					{
						...
						ImGui::Text("Mesafe: %f", GetDistanceFromLocation(l));
						ImGui::EndTooltip();
					}
				}
			}
