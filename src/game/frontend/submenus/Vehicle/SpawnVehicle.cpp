...
namespace YimMenu::Submenus
{
	...
	{
		auto tab = std::make_shared<TabItem>("Yeni Araç");

		auto spawn = std::make_shared<Group>("Doğurma");
		auto settings = std::make_shared<Group>("Ayarlar");
		...
		spawn->AddItem(std::make_unique<ImGuiItem>([] {
			...
			ImGui::InputTextWithHint("Ad", "Ara", search, sizeof(search));

			ImGui::SetNextItemWidth(300.f);
			if (ImGui::BeginCombo("Sınıf", selectedClass == -1 ? "Tümü" : g_VehicleClassNames[selectedClass]))
			{
				if (ImGui::Selectable("Tümü", selectedClass == -1))
				{
					selectedClass = -1;
				}
			...
			if (ImGui::BeginListBox("##vehicles", {300.f, height}))
			{
				...
				{
					ImGui::Text("Native'ler henüz önbelleğe alınmadı.");
				}
				else
				{
	...
	{
		auto tab = std::make_shared<TabItem>("Kişisel Araç");
		...
		spawn->AddItem(std::make_unique<ImGuiItem>([] {
			if (!*Pointers.IsSessionStarted)
				return ImGui::TextDisabled("GTA Online'a katıl.");
			...
			ImGui::SetNextItemWidth(300.f);
			if (ImGui::BeginCombo("Garaj", selectedGarageStr.empty() ? "Tümü" : selectedGarageStr.c_str()))
			{
				if (ImGui::Selectable("Tümü", selectedGarageStr.empty()))
				{
			...
			if (ImGui::BeginListBox("##personalvehicles", {300.f, height}))
			{
				...
				{
					ImGui::Text("İstatistikler henüz yüklenmedi.");
				}
				else
				{
								...
								FiberPool::Push([&personalVeh] {
									...
									else
									{
										if (!personalVeh->Request(spawnInsidePersonalVehicle.GetState()))
											Notifications::Show("Kişisel Araç Doğur", "Kişisel araç doğurulamadı.", NotificationType::Error);
									}
								});
							}
							ImGui::PopID();
	...
	{
		auto menu = std::make_shared<Category>("Doğurma");

		auto tabBar = std::make_shared<TabBarItem>("Doğurma");
