...
namespace YimMenu::Submenus
{
	std::shared_ptr<Category> BuildVehicleEditorMenu()
	{
		...
		vehicleEditor->AddItem(std::make_unique<ImGuiItem>([] {
			if (!Self::GetVehicle())
			{
				ImGui::Text("Lütfen bir araca bin.");
				currentVeh = 0;
				return;
			}
			...
			ImGui::BeginGroup();
			{
				...
				ImGui::SameLine();
				if (ImGui::Button("Mevcut Aracı Yenile"))
					FiberPool::Push([] {
						currentVeh = -1;
					});
				...
				ImGui::Spacing();
				{
					...
					ImGui::InputTextWithHint("##plate", "Plaka Numarası", plate, sizeof(plate), ImGuiInputTextFlags_None);
					ImGui::SameLine();
					if (ImGui::Button("Plakayı Değiştir"))
						FiberPool::Push([] {
							Self::GetVehicle().SetPlateText(plate);
						});
					ImGui::SameLine();
					if (ImGui::Button("Aracı Maksimumla"))
						FiberPool::Push([] {
							Self::GetVehicle().Upgrade();
							currentVeh = -1;
						});
				}
				ImGui::SeparatorText("Mod Seçenekleri");
				{
					if (ImGui::Checkbox("Patlayabilir lastikler", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_TIRE_CAN_BURST]))
						FiberPool::Push([] {
							VEHICLE::SET_VEHICLE_TYRES_CAN_BURST(currentVeh, owned_mods[(int)CustomVehicleModType::MOD_TIRE_CAN_BURST]);
						});
					ImGui::SameLine();
					if (ImGui::Checkbox("Düşük Tutuşlu Lastikler", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_DRIFT_TIRE]))
							...
							ImGui::BeginGroup();
							{
								...
								if (ImGui::BeginListBox("##style", ImVec2(200, 200)))
								{
									...
									for (int i = 0; i < wheel_mods.size(); i++)
									{
										...
										if (!isBennys)
										{
											if (i == 0 && ImGui::Selectable("Stok", mod == owned_mods[selected_slot] && *wheel_custom == 0))
												FiberPool::Push([&mod] {
													VEHICLE::SET_VEHICLE_MOD(currentVeh, selected_slot, mod, 0);
													currentVeh = -1;
												});
										...
										if (ImGui::Selectable(("Stil " + std::to_string(mod)).c_str(), mod == owned_mods[selected_slot] && *wheel_custom == should_custom))
				...
				ImGui::SeparatorText("Neon Işık Seçenekleri");
				{
					ImGui::PushID("##headlight_en");
					if (ImGui::Checkbox("Spot", (bool*)&owned_mods[(int)VehicleModType::MOD_XENON_LIGHTS]))
						FiberPool::Push([] {
							VEHICLE::TOGGLE_VEHICLE_MOD(currentVeh, (int)VehicleModType::MOD_XENON_LIGHTS, owned_mods[(int)VehicleModType::MOD_XENON_LIGHTS]);
						});
					ImGui::PopID();
					ImGui::SameLine();
					if (ImGui::Checkbox("Sol", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_NEON_LEFT_ON]))
						FiberPool::Push([] {
					...
					ImGui::SameLine();
					if (ImGui::Checkbox("Sağ", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_NEON_RIGHT_ON]))
						FiberPool::Push([] {
					...
					ImGui::SameLine();
					if (ImGui::Checkbox("Ön", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_NEON_FRONT_ON]))
						FiberPool::Push([] {
					...
					ImGui::SameLine();
					if (ImGui::Checkbox("Arka", (bool*)&owned_mods[(int)CustomVehicleModType::MOD_NEON_BACK_ON]))
						FiberPool::Push([] {
				...
				ImGui::SeparatorText("Renk Seçenekleri");
				{
					...
					{
						if (ImGui::Selectable("Birincil", color_to_change == 0))
							color_to_change = 0;
						if (ImGui::Selectable("İkincil", color_to_change == 1))
							color_to_change = 1;
						if (ImGui::Selectable("Sedef", color_to_change == 2))
						{
							color_to_change = 2;
							color_type = 7;
						}
						if (ImGui::Selectable("İç Mekan", color_to_change == 3))
						{
							color_to_change = 3;
							color_type = 10;
						}
						if (ImGui::Selectable("Gösterge Paneli", color_to_change == 4))
						{
							color_to_change = 4;
							color_type = 11;
						}
						...
						if (!owned_mods[(int)VehicleModType::MOD_TYRE_SMOKE])
							ImGui::BeginDisabled();
						if (ImGui::Selectable("Lastik Dumanı", color_to_change == 5))
						{
							color_to_change = 5;
							color_type = 9;
						}
						...
						if (ImGui::Selectable("Jant Rengi", color_to_change == 6))
						{
							color_to_change = 6;
							color_type = 8;
						}
						...
						if (ImGui::Selectable("Spot", color_to_change == 7))
						{
							color_to_change = 7;
							color_type = 12;
						}
					...
					if (color_to_change == 0 || color_to_change == 1)
					{
						...
						{
							if (ImGui::Selectable("Özel", color_type == 9))
								color_type = 9;

							if (ImGui::Selectable("Özel Rengi Kaldır", false))
								FiberPool::Push([] {
							...
							if (ImGui::Selectable("Krom", color_type == 0))
								color_type = 0;
							if (ImGui::Selectable("Klasik", color_type == 1))
								color_type = 1;
							if (ImGui::Selectable("Mat", color_type == 2))
								color_type = 2;
							if (ImGui::Selectable("Metalik", color_type == 3))
								color_type = 3;
							if (ImGui::Selectable("Faydalı", color_type == 4))
								color_type = 4;
							if (ImGui::Selectable("Yıpranmış", color_type == 5))
								color_type = 5;
							if (ImGui::Selectable("Bukalemun", color_type == 6))
								color_type = 6;
					...
					if (color_type == 9)
					{
						...
						if (ImGui::ColorPicker3("Özel Araç Rengi", color, ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_InputRGB | ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV | ImGuiColorEditFlags_InputHSV | ImGuiColorEditFlags_DisplayHex))
						{
					...
					else
					{
						...
						if (ImGui::BeginListBox("##color", ImVec2(180, 254)))
						{
							...
							case 0: //Chrome
							{
								if (ImGui::Selectable("Krom", selected_color == (int)VehicleColorsChrome::COLOR_CHROME))
								{
									if (color_to_change == 0)
							...
							case 8: //Wheel Color
							{
								...
								if (ImGui::Selectable("Alaşım", selected_color == alloy_color))
								{
