...
namespace YimMenu::Submenus
{
	std::shared_ptr<Category> BuildSpawnPedMenu()
	{
		...
		menu->AddItem(std::make_unique<ImGuiItem>([] {
								...
								if (spawnInMyVehicle)
								{
									...
									if (vehicle)
									{
										    ...
										    !vehicle.IsSeatFree(-1))
										{
											Notifications::Show(
											    "PED Doğur",
											    "PED aracın içine doğurulamadı, tüm koltuklar dolu. Lütfen önce bir koltuğu boşaltın veya 'Aracıma Doğur' seçeneğini kapatın.",
			...
			ImGui::SameLine();
			ImGui::BeginGroup();
			ImGui::BulletText("Ctrl+Click oyuncu modelini ayarlar");
			ImGui::Checkbox("Yenilmez", &invincible);
			ImGui::Checkbox("Ölü Doğur", &spawnDead);
			ImGui::Checkbox("Koruma Olarak Doğur", &spawnAsBodyguard);
			ImGui::Checkbox("Polis Olarak Doğur", &spawnAsCop);
			ImGui::Checkbox("Aracıma Doğur", &spawnInMyVehicle);
			ImGui::Checkbox("Tüm Silahları Ver", &giveAllWeapons);
			ImGui::Checkbox("Hayat Kadını Olarak Doğur", &spawnAsProstitute);
			ImGui::Checkbox("Kıyafeti Rastgeleleştir", &randomizeOutfit);
			ImGui::Checkbox("PED'i İşaretle", &blipPed);
			if (ImGui::Button("Tümünü Kaldır"))
			{
				FiberPool::Push([] {
